...
%                     P{cl}(:,:,k) = NormalizedSCM((X{cl}(:,:,k))'); %As Provided in Barachant toolbox 
        ...
        for testSession = 1:nbrSessions
            ...
            %- No riem potato
            COVtrain_filt = cat(3, P{2}(:,:,trainTrials), P{3}(:,:,trainTrials), P{4}(:,:,trainTrials));
            Ytrain_filt = [ones(1,length(trainTrials)) 2*ones(1,length(trainTrials)) 3*ones(1,length(trainTrials))];
...
%             eps = 0.01;
            ...
            types = [33025 33026 33027]; %-- For 3 classes
            ...
            for typ = 1:numel(types)
                ...
                class(typ,:) = (typ)*ones(size(pos(typ,:))); %-- For 3 classes
            end
            ...
            for tr = 1:Nt
                ...
                if ( ((F > thresh) && (grad(M)<eps)) ) %Check if identified class has occured more than the threshold and that the SCMs are moving toward this class, or whether the identified class is 0, in which case the gradient is not checked
                    Ytest(tr) = M;
                    delay_fin(tr) = N;
                    
                else
                    win = N+1;    
                    while ( (F <= thresh || grad(M) >= eps) && (win <= numel(delays)) ) % Check whether 1) the identified class has been majoritary in the last N data segments 2) whether the segments' SCM are moving into the direction of the identified class. And this can only be done within the available trial length determined by numel(delays)
            ...
            Ytest_all(testSession, :, sub) = Ytest;
            delay_fin_all(testSession, :, sub) = delay_fin;
            labels = CLASS';
            ac(sub, testSession) = sum((labels-Ytest)==0)/(trialPerSession*3- numel(find(Ytest==-1)));
...
resMatrix = [subId' subNbrOfSess' subAcMean' subStd'];
...
disp('---------------------------------------------------');
save('online_curve_3class.mat', 'itr', 'tLen2', 'resMatrix', 'resMean', 'ac', 'del_sub_all', 'subAcMean', 'delay_fin_all');
% conf = 0.7
