...
for k=1:gdfLnth
    ...
    for i=1:ClassLnth(1) % getting the no. of classes by taking the first element of ClassLnth 
        for j=1:ClassLnth(2) % this runs for the no. of trials in the gdf file
             X1{i,j}=BankSelect(X1(i,j),1,i,classes);
             temp=cell2mat(X1{i,j});
             C{i,j,k}=cov(temp); %Compute Covriance matrix of (class,trial) and store in C for all the gdf files in the folder
        end
