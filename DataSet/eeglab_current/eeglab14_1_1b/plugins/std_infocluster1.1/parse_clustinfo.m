...
%--------------------------------------------------------------------------
cellval = {STUDY.design(dsgn).cell.value}';                                % Change this lane
...
for icls = 1:length(cls)
    ...
    for nv1 = 1:ivar1
        for nv2 = 1:ivar2
            allinds   = STUDY.cluster(1,cls(icls)).allinds{nv1,nv2};
            setinds = STUDY.cluster(1,cls(icls)).setinds{nv1,nv2};
            tmpset = cell2mat({(STUDY.design(dsgn).cell(setinds).dataset)}');
            for i = 1:size(tmpset,1)
                SubjClusIC_Matrix(icls,tmpset(i,:),allinds(i)) = 1;
    ...
    clust_stat.clust(c1).iv{1}         = var1_dataval(store_sets)';
    clust_stat.clust(c1).iv{2}         = var2_dataval(store_sets)';
