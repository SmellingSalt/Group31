% runemd() - Perform Empirical Mode Decomposition (EMD) ,which was developed by Norden E. Huang (1998),
%            on the input data.
%            
% Usage:
%         >> [allmodes] = runemd(data); % train using defaults 
%    else
%         >> [allmodes  = runemd(data,'Key1',Value1',...);
% Input:
...
%
% Outputs:    The output is a matrix of N*(m+1) matrix, where N is the length of the input
% data , and m= number of modes . Column 1 is the original data, columns 2, 3, ...
...
function [allmodes]=runemd(chans,varargin)
...
disp('running.....................................................................................................................................');
  ...
  for chansnum=1:size(chans,1);
      ...
      Ystd = std(Y);
      Y = Y/Ystd;
      TNM = modes;     %; could be fix using : fix(log2(xsize))-1;
      TNM2 = TNM + 2;
      X1=zeros(xsize,1);
      for kk = 1:1:TNM2, 
        for ii = 1:1:xsize,
...
allmodechans(chansnum,:,:)=allmode;
clear allmode;
end;

%if size(chans,1)~=1

 k=size(allmodechans,3)/trials;
 k2=k;
 k1=1;
   for j=1:trials;

       ch(:,:,:,j)=allmodechans(:,:,k1:k);
