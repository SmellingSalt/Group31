% spectopoemd() - Plot the mean log spectrum of a set of data epochs for a specific mode at all channels 
...
% Usage:
%              >> spectopoemd(data, frames, srate);
%              >> [spectra,freqs,speccomp,contrib,specstd] = ...
%                    spectopoemd(data, frames, srate, 'key1','val1', 'key2','val2' ...);
% Inputs:
...
%
%   This code is written by Scott Makeig, Arnaud Delorme & Marissa Westerfield, 
%   SCCN/INC/UCSD, La Jolla, 3/01 and modified by Karema Al-Subari (Karema.Al-Subari@ur.de) 
%   and Saad Al-Baddai (Saad.Al-Baddai@ur.de).
%
% See also: timtopoemd(), envtopo(), tftopo(), topoplotemd()
...
% Uses: MATLAB pwelch(), changeunits(), topoplotemd(), textsc()

function [eegspecdB,freqs,compeegspecdB,resvar,specstd] = spectopoemd(data,frames,srate,varargin) 
...
if nargin <= 3 | isstr(varargin{1})
	...
	[g varargin] = finputcheck( varargin, fieldlist, 'spectopoemd', 'ignore');
	if isstr(g), error(g); end;
	if ~isempty(g.freqrange), g.limits(1:2) = g.freqrange; end;
	if ~isempty(g.weights)
		if isempty(g.freq) | length(g.freq) > 2
            if ~isempty(get(0,'currentfigure')) & strcmp(get(gcf, 'tag'), 'spectopoemd'), close(gcf); end;
         error('spectopoemd(): for computing component contribution, one must specify a (single) frequency');
		end;
	end;
else
    if ~isnumeric(data)
       error('spectopoemd(): Incorrect call format (see >> help spectopoemd).')
    end
	...
	if nargin > 4,	  g.emdchanlocs = varargin{2};
	else              g.emdchanlocs = [];
	end;
...
if ~isempty(g.freq) & isempty(g.emdchanlocs)
	error('spectopoemd(): needs channel location information');
end;
...
if ~isempty(g.freq) & min(g.freq)<0
    if ~isempty(get(0,'currentfigure')) & strcmp(get(gcf, 'tag'), 'spectopoemd'), close(gcf); end;
   fprintf('spectopoemd(): freqs must be >=0 Hz\n');
   return
end

g.emdchanlocs2 = g.emdchanlocs;
if ~isempty(g.specdata)
    ...
    if frames*epochs ~= size(data,2)
       error('spectopoemd: non-integer number of epochs');
    end
    if ~isempty(g.weights)
    ...
    if isempty(g.weights)
        ...
        if length(tmpc) ~= size(eegspecdB,1)
            ...
            if ~isempty(g.emdchanlocs)
                g.emdchanlocs2 = g.emdchanlocs(tmpc);
            end
        end;
...
if isnan(g.limits(5))+isnan(g.limits(6)) == 1
   fprintf('spectopoemd(): limits 5 and 6 must either be given or nan\n');
   return
end
...
if ~isempty(g.freq) &  strcmpi(g.plot, 'on')
	...
	for f=1:length(g.freq)
        ...
        if ~isempty(g.plotchan) & g.plotchan ~= 0 
			% if ~isempty(varargin) % if there are extra topoplotemd() flags
			%	topoplotemd(g.plotchan,g.emdchanlocs,'electrodes','off', ...
			%			 'style', 'blank', 'emarkersize1chan', 10, varargin{:});
			% else
				topoplotemd(g.plotchan,g.emdchanlocs,'electrodes','off', ...
						 'style', 'blank', 'emarkersize1chan', 10);
			% end
			if isstruct(g.emdchanlocs)
				tl=title(g.emdchanlocs(g.plotchan).labels);
			else
			...
			if ~isempty(varargin)
				topoplotemd(topodata(g.mapframes),g.emdchanlocs2,'maplimits',maplimits, varargin{:}); 
			else
				topoplotemd(topodata(g.mapframes),g.emdchanlocs2,'maplimits',maplimits); 
			end
			if f<length(g.freq)
	...
	if ~isempty(g.weights)
		...
		for index = 1:length(g.icamaps)
			...
			topoplotemd(g.icawinv(g.mapchans,compnum).^2,g.emdchanlocs,varargin{:}); 
...
%%%%%%%%%%%%%%%%
if strcmpi(g.plot, 'on')
    disp('Click on each trace for channel index');
