% eeg_checksetemd()   - check the consistency of the fields of an EEG dataset
%                    Also: See EEG dataset structure field descriptions below.
%
% Usage: >> [EEGOUT,changes] = eeg_checksetemd(EEG); % perform all checks
%                                                  except 'makeur'
%        >> [EEGOUT,changes] = eeg_checksetemd(EEG, 'keyword'); % perform 'keyword' check(s)
%
% Inputs:
...
%   'emd'          - check whether EEG contains an EMD decomposition
...
%
%  The code is written by  Arnaud Delorme, CNL / Salk Institute, 2001
%  and modified by Karema Al-Subari (Karema.Al-Subari@ur.de) and Saad Al-Baddai (Saad.Al-Baddai@ur.de).
%
...
function [EEG, res] = eeg_checksetemd( EEG, varargin );

msg = '';
res = 'no';
com = sprintf('EEG = eeg_checksetemd( EEG );');

if nargin < 1
    help eeg_checksetemd;
    return;
end;
...
for inddataset = 1:length(ALLEEG)
    ...
    if ~isempty( varargin)
        for index = 1:length( varargin )
            switch varargin{ index }
                ...
                case 'emd_epoch',
                    if ~isfield( EEG,'IMFs' ) | isempty(EEG.IMFs)
                        errordlg2(strvcat('Error: no EMD decomposition. use menu "Tools > Run EMD" first.'), 'Error');
                        return;
                    end;
                ...
                case 'chanlocs_homogeneous',
                    ...
                    if ~isfield(EEG.chanlocs, 'X') || isempty(EEG.chanlocs(1).X)
                        ...
                        res = [ inputname(1) ' = eeg_checksetemd('  inputname(1) ', ''chanlocs_homogeneous'' ); ' ];
                    end;
                case 'chanlocsize',
                ...
                case 'makeur',
                    if ~isempty(EEG.event)
                        if isfield(EEG.event, 'urevent'),
                            ...
                            disp('eeg_checksetemd note: re-creating the original event table (EEG.urevent)');
                        else
                            disp('eeg_checksetemd note: creating the original event table (EEG.urevent)');
                        end;
                ...
                case 'eventconsistency',
                    ...
                    if isfield(EEG.event, 'latency')
                        ...
                        if (length(I1) + length(I2)) > 0
                            fprintf('eeg_checksetemd warning: %d/%d events had out-of-bounds latencies and were removed\n', ...
                    ...
                    if ~isempty(EEG.event) && isfield(EEG.event, 'epoch')
                        ...
                        try, tmpevent = EEG.event; allepochs = [ tmpevent.epoch ];
                            ...
                            if ~isempty(removeevent)
                                disp([ 'eeg_checksetemd warning: ' int2str(length(removeevent)) ' event had invalid epoch numbers and were removed']);
                            end;
                        catch,
                            for indexevent = 1:length(EEG.event)
                                if isempty( EEG.event(indexevent).epoch ) || ~isnumeric(EEG.event(indexevent).epoch) ...
                                    ...
                                    disp([ 'eeg_checksetemd warning: event ' int2str(indexevent) ' has an invalid epoch number: removed']);
                                end;
                            end;
                        end;
                        ...
                        for index = 1:length(difffield)
                            ...
                            if strcmp( difffield{index}, 'duration')
                                if any(valempt)
                                    fprintf(['eeg_checksetemd: found empty values for field ''' difffield{index} ...
                                        ''' (filling with 0)\n']);
                                end;
                    ...
                    for fidx = 1:length(fnames)
                        ...
                        if strcmp(format, 'str')
                            fprintf('eeg_checksetemd note: value format of event field ''%s'' made uniform\n', fname);
                            % get the field content
                            % ---------------------
                            for indexevent = 1:length(EEG.event)
                    ...
                    if isfield(EEG.event, 'latency')
                        try,
                            ...
                            disp('eeg_checksetemd: problem when attempting to resort event latencies.');
                        end;
                    end;
                ...
                otherwise, error('eeg_checksetemd: unknown option');
            end;
        end;
    end;
    ...
    if ~isempty(EEG.setname)
        if ~ischar(EEG.setname)
            ...
            if size(EEG.setname,1) > 1
                disp('eeg_checksetemd warning: invalid dataset name, removed');
                EEG.setname = '';
            end;
        end;
    else
    ...
    if nargin > 1
        ...
        if strcmpi(varargin{1}, 'savedata') & option_storedisk
            error('eeg_checksetemd: cannot call savedata any more');
            ...
            if ~ischar(EEG.data) % not already saved
                ...
                res = sprintf('%s = eeg_checksetemd( %s, ''savedata'');', inputname(1), inputname(1));
            end;
        end;
    end;
    ...
    if isnumeric(EEG.data) && ~isempty(EEG.data)
        if ~isequal(size(EEG.data,1), EEG.nbchan)
            disp( [ 'eeg_checksetemd warning: number of columns in data (' int2str(size(EEG.data,1)) ...
        ...
        if (ndims(EEG.data)) < 3 & (EEG.pnts > 1)
            if mod(size(EEG.data,2), EEG.pnts) ~= 0
                if popask( [ 'eeg_checksetemd error: the number of frames does not divide the number of columns in the data.'  10 ...
                        'Should EEGLAB attempt to abort operation ?' 10 '(press Cancel to fix the problem from the command line)'])
                    error('eeg_checksetemd error: user abort');
                    %res = com;
                    ...
                    res = com;
                    return;
                    %error( 'eeg_checksetemd error: number of points does not divide the number of columns in data');
                end;
            else
                if EEG.trials > 1
                    disp( 'eeg_checksetemd note: data array made 3-D');
                    res = com;
                end;
        ...
        if size(EEG.data,3) ~= EEG.trials
            disp( ['eeg_checksetemd warning: 3rd dimension size of data (' int2str(size(EEG.data,3)) ...
                ') does not match the number of epochs (' int2str(EEG.trials) '), corrected' ]);
            res = com;
            EEG.trials = size(EEG.data,3);
        end;
        if size(EEG.data,2) ~= EEG.pnts
            disp( [ 'eeg_checksetemd warning: number of columns in data (' int2str(size(EEG.data,2)) ...
    ...
    if round(EEG.srate*(EEG.xmax-EEG.xmin)+1) ~= EEG.pnts
        fprintf( 'eeg_checksetemd note: upper time limit (xmax) adjusted so (xmax-xmin)*srate+1 = number of frames\n');
        if EEG.srate == 0
            EEG.srate = 1;
        end;
    ...
    if ~isempty(EEG.event)
        if EEG.trials > 1 & ~isfield(EEG.event, 'epoch')
            if popask( [ 'eeg_checksetemd error: the event info structure does not contain an ''epoch'' field.'  ...
                ...
                res = com;
                return;
                %error('eeg_checksetemd error: no epoch field in event structure');
            end;
        end;
    else
        EEG.event = [];
    ...
    if ~isempty(EEG.epoch)
        ...
        if l ~= EEG.trials
            if popask( [ 'eeg_checksetemd error: the number of epoch indices in the epoch array/struct (' ...
                ...
                res = com;
                return;
                %error('eeg_checksetemd error: epoch structure size invalid');
            end;
        end;
    else
        EEG.epoch = [];
    ...
    if ~isempty(EEG.icasphere)
        if ~isempty(EEG.icaweights)
            if size(EEG.icaweights,2) ~= size(EEG.icasphere,1)
                if popask( [ 'eeg_checksetemd error: number of columns in weights array (' int2str(size(EEG.icaweights,2)) ')' 10 ...
                    ...
                    res = com;
                    return;
                    %error('eeg_checksetemd error: invalid weight and sphere array sizes');
                end;
            end;
            if isnumeric(EEG.data)
                if length(EEG.icachansind) ~= size(EEG.icasphere,2)
                    if popask( [ 'eeg_checksetemd error: number of elements in ''icachansind'' (' int2str(length(EEG.icachansind)) ')' 10 ...
                ...
                if isempty(EEG.icaact) | (size(EEG.icaact,1) ~= size(EEG.icaweights,1)) | (size(EEG.icaact,2) ~= size(EEG.data,2))
                    ...
                    if ~isempty(EEG.data) && option_computeica
                        fprintf('eeg_checksetemd: recomputing the ICA activation matrix ...\n');
                        res = com;
                        % Make compatible with Matlab 7
                        if any(isnan(EEG.data(:)))
                            tmpdata = EEG.data(EEG.icachansind,:);
                            fprintf('eeg_checksetemd: recomputing ICA ignoring NaN indices ...\n');
            ...
            disp( [ 'eeg_checksetemd warning: weights matrix cannot be empty if sphere matrix is not, correcting ...' ]);
            res = com;
            EEG.icasphere = [];
        end;
        if option_computeica
            if ~isempty(EEG.icaact) & ndims(EEG.icaact) < 3 & (EEG.trials > 1)
                disp( [ 'eeg_checksetemd note: independent component made 3-D' ]);
                res = com;
                EEG.icaact = reshape(EEG.icaact, size(EEG.icaact,1), EEG.pnts, EEG.trials);
            end;
        else
            if ~isempty(EEG.icaact)
                fprintf('eeg_checksetemd: removing ICA activation matrix (as per edit options) ...\n');
            end;
            EEG.icaact     = [];
        end;
    else
    ...
    if ~isempty( EEG.chanlocs )
        ...
        if ~isstruct( EEG.chanlocs)
            if exist( EEG.chanlocs ) ~= 2
                disp( [ 'eeg_checksetemd warning: channel file does not exist or is not in Matlab path: filename removed from EEG struct' ]);
                EEG.chanlocs = [];
                res = com;
            else
                res = com;
                try, EEG.chanlocs = readlocs( EEG.chanlocs );
                    disp( [ 'eeg_checksetemd: channel file read' ]);
                catch, EEG.chanlocs = []; end;
            end;
        else
            if ~isfield(EEG.chanlocs,'labels')
                disp('eeg_checksetemd warning: no field label in channel location structure, removing it');
                EEG.chanlocs = [];
                res = com;
            end;
        end;
        if isstruct( EEG.chanlocs)
            if length( EEG.chanlocs) ~= EEG.nbchan && length( EEG.chanlocs) ~= EEG.nbchan+1 && ~isempty(EEG.data)
                disp( [ 'eeg_checksetemd warning: number of channels different in data and channel file/struct: channel file/struct removed' ]);
                EEG.chanlocs = [];
                res = com;
            end;
        end;
    ...
    %    end;
    %    disp('eeg_checksetemd note: creating backup chanlocs structure (urchanlocs)');
    %end;
