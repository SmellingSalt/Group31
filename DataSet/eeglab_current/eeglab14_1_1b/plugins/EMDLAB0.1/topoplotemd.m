% topoplotemd() - plot a topographic map of a scalp data field in a 2-D circular view 
...
% Usage:
%        >>  topoplotemd(datavector, EEG.chanlocs);   % plot a map using an EEG chanlocs structure
%        >>  topoplotemd(datavector, 'my_chan.locs'); % read a channel locations file and plot a map
%        >>  topoplotemd('example');                  % give an example of an electrode location file
%        >>  [h grid_or_val plotrad_or_grid, xmesh, ymesh]= ...
%                           topoplotemd(datavector, chan_locs, 'Input1','Value1', ...);
% Required Inputs:
%   datavector        - single vector of channel's mode values. Else, if a vector of selected subset
%                       (int) channel numbers -> mark their location(s) using 'style' 'blank'.
%   chan_locs         - name of an EEG electrode position file (>> topoplotemd example).
%                       Else, an EEG.chanlocs structure (>> help readlocs or >> topoplotemd example)
% Optional inputs:
...
%                        >> topoplotemd(values,'chanlocs','plotgrid',[11 12 0; 13 14 15]);
...
%                       See >> topoplotemd example. If plotrad > 0.5, chans with arc_length > 0.5 
...
%                       see >> topoplotemd 'example' {default: 'off'}
...
%   'pmask'           - values for masking topoplotemd. Array of zeros and 1 of the same size as the input 
%                       value array {default: []}
...
%
% Chan_locs format:
%    See >> topoplotemd 'example'
%
% Examples:
%
%    To plot channel locations only:
%    >> figure; topoplotemd([],EEG.chanlocs,'style','blank','electrodes','labelpoint','chaninfo',EEG.chaninfo);
%    
...
%
%  This code is written by Andy Spydell, Colin Humphries, Arnaud Delorme & Scott Makeig
%  CNL / Salk Institute, 8/1996-/10/2001; SCCN/INC/UCSD, Nov. 2001 and modified by
...
function [handle,Zi,grid,Xi,Yi] = topoplotemd(Values,loc_file,varargin)
...
if fieldtrip
    ...
    addpath(dir2);
    try,
        topoplotemd(Values, loc_file, varargin{:});
    catch, 
    end;
    addpath(dir1);
    return;
  ...
  if isstr(Values)
    if any(strcmp(lower(Values),{'example','demo'}))
      fprintf(['This is an example of an electrode location file,\n',...
               ...
               '13               180    .181       Pz  \n\n',...
                                                             ...
               'In topoplotemd() coordinates, 0 deg. points to the nose, positive\n',...
...
if nargs < 2
  ...
  if ~exist(loc_file)
      fprintf('default locations file "%s" not found - specify chan_locs in topoplotemd() call.\n',loc_file)
      error(' ')
  end
end
if isempty(loc_file)
...
if strcmp(plotgrid,'on')
   ...
   %      fprintf('topoplotemd() warning: ''plotgrid'' channel matrix has duplicate channels\n');
   % end

   if ~isempty(plotchans)
     if intersect(gchans,abs(plotchans))
        fprintf('topoplotemd() warning: ''plotgrid'' and ''plotchans'' have channels in common\n');
     end
   end
end
...
if isempty(headrad)  % never set -> defaults
  if plotrad >= rmax
     ...
     if strcmpi(VERBOSE, 'on')
       fprintf('topoplotemd(): not plotting cartoon head since plotrad (%5.4g) < 0.5\n',...
                                                                    plotrad);
     end
  end
...
% fprintf('topoplotemd(): plotting %d channels\n',length(pltchans));
 ...
 if ~strcmpi(STYLE,'grid') %  not a rectangular channel grid
  ...
  %
   if strcmpi(noplot, 'on') 
    if strcmpi(VERBOSE,'on')
       fprintf('topoplotemd(): no plot requested.\n')
    end
    return;
   end
  %
