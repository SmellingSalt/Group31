% pop_headplotemd() - plot one or more spherically-splined EEG field maps 
...
% Usage:
%   To open input GUI:
%   >> EEGOUT = pop_headplotemd( EEG)
%   To run as a script without further GUI input:
%   >> EEGOUT = pop_headplotemd( EEG, typeplot, ...
...
%   latencies  - If channels, array of epoch mean latencies (in ms),
%                
%
% Optional inputs:
%   title      - Plot title
...
%   'setup'    - ['name_of_file_to_save.spl'] Make the headplotemd spline file
%   'load'     - ['name_of_file_to_load.spl'] Load the headplotemd spline file
%   'colorbar' - ['on' or 'off'] Switch to turn colorbar on or off. {Default: 'on'}
%   others...  - All other key-val calls are passed directly to headplotemd. 
%                See >> help headplotemd
%
% Output:
...
%
%   headplotemd spline file is a matlab .mat file with the extension .spl.
%
%  This code is written by Arnaud Delorme, CNL / Salk Institute, 2002 and modified by
%  Karema Al-Subari (Karema.Al-Subari@ur.de) and Saad Al-Baddai (Saad.Al-Baddai@ur.de).
%
%
% See also: headplotemd(), eegplot(), traditional()
...
function [com] = pop_headplotemd( EEG, arg2, topotitle, rowcols, varargin);
...
if nargin < 1
   help pop_headplotemd;
   return;
end;   

if isempty(EEG.emdchanlocs)
    error('Pop_headplotemd: this dataset does not contain channel locations. Use menu item: Edit > Dataset info');
end;
...
if nargin < 3 % Open GUI input window
    ...
    if isfield(EEG, 'splinefile') 
        if ~isempty(EEG.splinefile) && exist(EEG.splinefile, 'file')
            ...
            byteperelec = splfile.bytes/EEG.emdnbchan;
            if byteperelec/EEG.emdnbchan < 625, % old head plot file
                EEG.splinefile = [];
    ...
    if typeplot == 1 % ********** data plot
        ...
        if isempty(EEG.splinefile) && exist(EEG.splinefile, 'file')          
            if length(EEG.icachansind) == EEG.emdnbchan & ~isempty(EEG.icasplinefile)
        ...
        if isempty(EEG.icasplinefile) && exist(EEG.icasplinefile, 'file')
            if length(EEG.icachansind) == EEG.emdnbchan & ~isempty(EEG.splinefile)
    ...
    userdatatmp = { EEG.emdchanlocs EEG.chaninfo };
    ...
    { 'style' 'text' 'String' 'Select Mode:' 'fontweight' 'bold'}, ...
    ...
    { },...
	        ...
	        { 'style' 'edit' 'string' [ fastif( EEG.trials > 1 , 'ERM scalp maps of dataset:', 'IMF scalp maps of dataset:') ...
    ...
    % -------------------------------
    geom = { [1] [1.3 1.6 0.5 0.5] [1.3 1.6 0.5 0.5 ] [1.3 1.6 0.5 0.5 ] [1.3 1.6 0.5 0.5 ] [1.3 1.6 0.5 0.5 ] [1.3 1.6 0.5 0.5 ] ...
             [1] [1] [1.5 1 0.5] [1.5 1  0.5] [1.5 1  0.5] [1.5 1 0.5] };
    optiongui = { 'uilist', txt, 'title', fastif( typeplot, 'ERM head plot(s) -- pop_headplotemd()', ...
                       'Modes head plot(s) -- pop_headplotemd()'), 'geometry', geom };
    ...
    % -----------------------
    options = {};
    if result{2},               options = { options{:} 'load'    result{3} };
    else
        if isempty(result{8})   setupopt = { result{5} 'meshfile' result{6} };  % no coreg
        else                    setupopt = { result{5} 'meshfile' result{6} 'transform' str2num(result{8}) };
                                fprintf('Transformation matrix: %s\n', result{8});
        end;
        options = { options{:} 'setup' setupopt };
        if ~strcmpi(result{6}, 'mheadnew.mat'), EEG.headplotmeshfile = result{6}; end;
    end;
	...
	topotitle  = result{10};
	rowcols    = eval( [ '[ ' result{11} ' ]' ] );
    tmpopts    = eval( [ '{ ' result{12} ' }' ] );
    if ~isempty(tmpopts)
...
if ~isempty(loc)
        ...
        headplot('setup', EEG.emdchanlocs, options{loc+1}{1}, 'chaninfo', EEG.chaninfo, options{ loc+1 }{2:end});
...
if ~exist('topotitle')  
    ...
    if isempty(EEG.splinefile)
        error('Pop_headplotemd: cannot find spline file, aborting...');
    end;
...
% -----------------------
com = sprintf('pop_headplotemd(%s, %d, %s, ''%s'', [%s], %s);', inputname(1), typeplot, vararg2str(arg2), ...
    ...
    if ~loadfile
        ...
        if isempty(coreg1)
            textlines = strvcat('You must co-register your channel locations with the head model.', ...
                                ...
                                'right center of the pop_headplotemd() window and follow instructions.',...
