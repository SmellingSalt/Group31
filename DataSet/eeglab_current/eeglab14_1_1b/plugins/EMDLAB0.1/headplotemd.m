% headplotemd() - plot a spherically-splined EEG field map on a semi-realistic 
%              3-D head model. Can 3-D rotate the head image using the left 
%              mouse button.
% Example:
%   >> headplotemd example   % show an example spherical 'eloc_angles' file
%   >> headplotemd cartesian % show an example cartesian 'eloc_angles' file
%
% Setup usage (do only once for each scalp montage):
%
%   >> headplotemd('setup', elocs, splinefile, 'Param','Value',...);
%      %
%      % NOTE: previous call format below is still supported
%      %       >> headplotemd('setup', elocs, splinefile, comment, type);
%
...
%
%       >> headplotemd(values,'spline_file','Param','Value',...)
%
...
%                   h -> Colorbar axis handle (to specify headplotemd location)
...
%
% Note: if an error is generated, headplotemd() may close the current figure
%
%  This code is written by Arnaud Delorme, Colin Humphries, Scott Makeig, SCCN/INC/UCSD, 
%  La Jolla, 1998 and modified by  Karema Al-Subari (Karema.Al-Subari@ur.de) 
...
function [HeadAxes, ColorbarHandle] = headplotemd(values, arg1, varargin)
...
%
    ...
    fprintf('headplotemd: using existing XYZ coordinates\n');
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if ~exist(g.meshfile)
       error(sprintf('headplotemd(): mesh file "%s" not found\n',g.meshfile));
    end
    ...
    try, save(spline_file, '-V6', '-mat', 'Xe', 'Ye', 'Ze', 'G', 'gx', 'newElect', ...
        ...
        try,  save(spline_file, '-mat', 'Xe', 'Ye', 'Ze', 'G', 'gx', 'newElect', ...
                   'ElectrodeNames', 'indices', 'comment', 'headplot_version', 'transform');
        catch, error('headplotemd: save spline file error, out of space or file permission problem');
        end;
    end;
...
%
       fprintf(['\nExample of a headplotemd() electrode angles file (spherical coords.)\n',...
             ...
             '\nA 90 deg coronal rotation points to right ear, -90 to left.\n' ,...
               ...
               'See also >> headplotemd cartesian\n\n\n']);
       return
  elseif strcmp(values,'cartesian') 
%
%%%%%%%%%%%%%%%%%% Show an example cartesian electrode file  %%%%%%%%%%%%%%%%%%%
%
       fprintf(['\nExample of a headplotemd() electrode location file (cartesian coords.)\n',...
               'Fields:  chan_num  x        y        z      channel_name\n\n',...
               '           1       0.4528   0.8888  -0.0694       Fp1.\n',...
               'Channel names should have 4 chars (. = space).\n',...
               'See also >> headplotemd example\n\n\n']);
       return
  else
    fprintf('headplotemd(): Unknown first argument (%s).\n',values)
    help headplotemd
  end
else
%
...
%   
   ...
   g = finputcheck( varargin, { ...
       ...
       'labels'     'integer' [0 1 2]        0 }, 'headplotemd');
  ...
  if ~exist(spline_file)
       error(sprintf('headplotemd(): spline_file "%s" not found. Run headplotemd in "setup" mode\n',...
           spline_file));
  end
  ...
  if enum ~= length(Xe)
	  close;
	  error('headplotemd(): Number of values in spline file should equal number of electrodes')
  end
  ...
  if isstr(g.view)
    switch lower(g.view)
      ...
      otherwise
        close; error(['headplotemd(): Invalid View value %s',g.view])
    end
  else
      if ~isstr(g.view)
          [h,a] = size(g.view);
          if h~= 1 | a~=2
              close; error('headplotemd(): View matrix size must be (1,2).')
          end
      end
