% eegplotemd() - Scroll (horizontally and/or vertically) through multichannel data and their modes.
%             Allows vertical scrolling through channels.
% Usage: 
%           >> eegplotemd(data, 'key1', value1 ...); % use interface buttons, etc.
%      else
%           >> eegplotemd('noui', data, 'key1', value1 ...); % no user interface;
%                                                         % use for plotting
% Menu items:
...
%                  button is pressed (see Outputs); called from pop_eegplotemd() or 
...
%                  pop_eegplotemd(), the channel labels of the dataset will be used. 
...
%    'tag'        - [string] Matlab object tag to identify this eegplotemd() window (allows 
%                    keeping track of several simultaneous eegplotemd() windows). 
%    'children'   - [integer] Figure handle of a *dependent* eegplotemd() window. Scrolling
...
%
%  This code is written by Arnaud Delorme & Colin Humphries, CNL/Salk Institute, SCCN/INC/UCSD, 1998-2001
...
function [outvar1] = eegplotemd(data, varargin); % p1,p2,p3,p4,p5,p6,p7,p8,p9)
...
% %%%%%%%%%%%%%%%%%%%%%%%%
 EEG= evalin('base','EEG');
for i=1:size(EEG.IMFs,2)
if i==1
 names{i}=strcat('Original'); % fil
 elseif i==size(EEG.IMFs,2)
 names{i}=strcat('Resdium'); % fil
else
 names{i}=strcat('IMF_',num2str(i-1)); % fil
end
 end
...
if ~isstr(data) % If NOT a 'noui' call or a callback from uicontrols
   ...
   %catch
   %		disp('eegplotemd() error: calling convention {''key'', value, ... } error'); return;
   %end;	
  ...
  % ---------------------------------
  defdowncom   = 'eegplotemd(''defdowncom'',   gcbf);'; % push button: create/remove window
  defmotioncom = 'eegplotemd(''defmotioncom'', gcbf);'; % motion button: move windows or display current position
  defupcom     = 'eegplotemd(''defupcom'',     gcbf);';
  defctrldowncom = 'eegplotemd(''topoplot'',   gcbf);'; % CTRL press and motion -> do nothing by default
  ...
  defctrlupcom = ''; % CTRL press and up -> do nothing by default
   ...
   for index=1:length(gfields)
      ...
      otherwise, error(['eegplotemd: unrecognized option: ''' gfields{index} '''' ]);
      end;
   end;
  ...
  if isstr(g.eloc_file) | isstruct(g.eloc_file)  % Read in electrode names
      if isstruct(g.eloc_file) & length(g.eloc_file) > size(data,1)
        ...
        eegplotemd('setelect', g.eloc_file, ax1);
  end;
...
% positions of buttons
  ...
  posbut(23,:) = [0.50   0.92     0.1      0.048];%select modes
  posbut(24,:) = [0.608   0.93     0.1      0.048];%modes
...
% Five move buttons: << < text > >> 
  ...
  u(1) = uicontrol('Parent',figh, ...
	...
	'Tag','Pushbutton1',...
	'string','<<',...
	'Callback','eegplotemd(''drawp'',1)');
  u(2) = uicontrol('Parent',figh, ...
	'Units', 'normalized', ...
	'Position', posbut(2,:), ...
	'Tag','Pushbutton2',...
	'string','<',...
	'Callback','eegplotemd(''drawp'',2)');
  u(5) = uicontrol('Parent',figh, ...
	...
	'Callback', 'eegplotemd(''drawp'',0);' );
  u(3) = uicontrol('Parent',figh, ...
	'Units', 'normalized', ...
	'Position',posbut(3,:), ...
	'Tag','Pushbutton3',...
	'string','>',...
	'Callback','eegplotemd(''drawp'',3)');
  u(4) = uicontrol('Parent',figh, ...
	'Units', 'normalized', ...
	'Position',posbut(4,:), ...
	'Tag','Pushbutton4',...
	'string','>>',...
	'Callback','eegplotemd(''drawp'',4)');
...
u(24) = uicontrol('Parent',figh, ...
  ...
  if ~isempty(g.events)
      u(17) = uicontrol('Parent',figh, ...
                        'Units', 'normalized', ...
                        'Position',posbut(17,:), ...
                        'string', 'Event types', 'callback', 'eegplotemd(''drawlegend'', gcbf)');
  end;
  ...
  uimenu('Parent',m(7),'Label','Edit figure', 'Callback', 'eegplotemd(''noui'');');
%   uimenu('Parent',m(7),'Label','Accept and close', 'Callback', acceptcommand );
  ...
  % Scale Eye %%%%%%%%%
  timestring = ['[OBJ1,FIG1] = gcbo;',...
	        'eegplotemd(''scaleeye'',OBJ1,FIG1);',...
		'clear OBJ1 FIG1;'];
  m(7) = uimenu('Parent',m(1),'Label','Show scale','Callback',timestring);
  
  % Title %%%%%%%%%%%%
  uimenu('Parent',m(1),'Label','Title','Callback','eegplotemd(''title'')')
  ...
  uimenu('Parent',m(2),'Label','Time range to display',...
      'Callback','eegplotemd(''window'')')
  
  % Electrode window %%%%%%%%
  uimenu('Parent',m(2),'Label','Number of channels to display',...
      'Callback','eegplotemd(''winelec'')')
  ...
  uimenu('Parent',m(6),'Label','Load .loc(s) file',...
      'Callback','eegplotemd(''loadelect'');')
  
  % Zooms %%%%%%%%
  zm = uimenu('Parent',m(2),'Label','Zoom off/on');
  commandzoom = [ 'set(gcbf, ''windowbuttondownfcn'', [ ''zoom(gcbf,''''down''''); eegplotemd(''''zoom'''', gcbf, 1);'' ]);' ...
  ...
  uimenu('Parent',zm,'Label','Zoom off', 'separator', 'on', 'callback', ...
     ...
     'clear tmpg;' ]);
  uimenu('Parent',figh,'Label', 'Help', 'callback', 'pophelp(''eegplotemd'');');

  % Events %%%%%%%%
  zm = uimenu('Parent',m(2),'Label','Events');
  complotevent = [ 'tmpg = get(gcbf, ''userdata'');' ...
                  'tmpg.plotevent = ''on'';' ...                  
                  'set(gcbf, ''userdata'', tmpg); clear tmpg; eegplotemd(''drawp'', 0);'];
  comnoevent   = [ 'tmpg = get(gcbf, ''userdata'');' ...
                  'tmpg.plotevent = ''off'';' ...                  
                  'set(gcbf, ''userdata'', tmpg); clear tmpg; eegplotemd(''drawp'', 0);'];
  comeventleg  = [ 'eegplotemd(''drawlegend'', gcbf);'];
...
%  set(figh, 'busyaction', 'cancel');
  ...
  eegplotemd('scaleeye', [], gcf);
  if strcmp(lower(g.scale), 'off')
	  eegplotemd('scaleeye', 'off', gcf);
  end;
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  case 'drawp' % Redraw EEG and change position
    ...
    % this test help to couple eegplotemd windows
    if exist('p3')
    	figh = p3;
    	figure(p3);
    else	
    ...
    g = get(figh,'UserData');
    E= findobj('tag','modes','parent',gcf);   % ui handle
    items = get(E, 'value');
    EEG= evalin('base','EEG');
...
if size(EEG.IMFs,1)==1
   data(1,:,:)=squeeze(EEG.IMFs(1,items,:,:));
   else
   data=squeeze(EEG.IMFs(:,items,:,:));
  end
    ...
    % channels in the eegplotemd figure window.
    for i = 1:g.chans
...
%        keyboard;  
    ...
    set(ax1, 'ylim',[g.elecoffset*g.spacing (g.elecoffset+g.dispchans+1)*g.spacing] );
	 ...
	 if g.children ~= 0
		...
		eegplotemd( 'drawp', p1, p2, g.children);
		figure(figh);
	 end;	  
...
{ 'Number of channels to display:' } , 'Change number of channels to display', 1,  { num2str(g.dispchans) });
   ...
   set(gcf, 'UserData', g);
   eegplotemd('updateslider', fig);
   eegplotemd('drawp',0);	
   eegplotemd('scaleeye', [], fig);
   return;
  ...
  case 'loadelect' % load channels
	...
	eegplotemd('setelect',[ inputpath inputname ],AXH0);
	return;
  ...
  case 'title'
	...
	if ~isempty(h)
		...
		if ~isempty(result), h = textsc(result{1}, 'title'); set(h, 'tag', 'eegplotemdtitle');end;
	end;
  ...
  case 'noui'
      if ~isempty(varargin)
          eegplot_noui( varargin{:} ); fig = gcf;
      else 
          fig = findobj('tag', 'eegplotemd');
      end;
   ...
   case 'zoom' % if zoom
      ...
      % ------------------------------
      if exist('p2') == 1
          set(gcbf, 'windowbuttondownfcn', [ 'zoom(gcbf,''down''); eegplotemd(''zoom'', gcbf, 1);' ]);
    ...
    if strcmp(get(fig, 'SelectionType'),'normal');
        ...
        if g.incallback ~= 1 % interception of nestest calls
            ...
            if (tmppos(1) >= 0) & (tmppos(1) <= highlim),
                ...
                set(fig,'UserData', g);
                eegplotemd('drawp', 0);  % redraw background
            end;
        end;
    end;      
   otherwise
      error(['Error - invalid eegplotemd() parameter: ',data])
  end  
end
