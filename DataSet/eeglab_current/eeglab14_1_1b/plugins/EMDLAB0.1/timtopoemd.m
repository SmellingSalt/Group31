% timtopoemd()   - plot a specific mode of all channels of a data epoch on the same axis 
%               and map its scalp map(s) at selected latencies.
% Usage:
%  >> figure; timtopoemd(data, chan_locs);
%  >> figure; timtopoemd(data, chan_locs, 'key', 'val', ...);
% Inputs:
%  data       = (a mode of channels,frames) single-epoch data matrix
%  chan_locs  = channel location file or EEG.chanlocs structure. 
%               See >> topoplotemd example for file format.
%
...
%
%  This code is written by Scott Makeig, SCCN/INC/UCSD, La Jolla, 1-10-98 and modified by
%  Karema Al-Subari (Karema.Al-Subari@ur.de) and Saad Al-Baddai (Saad.Al-Baddai@ur.de).
% 
%
% See also: envtopo(), topoplotemd()
...
function M = timtopoemd(data, chan_locs, varargin)

MAX_TOPOS = 24;

if nargin < 2 % should this be 2?
   help timtopoemd;
   return
end
...
fieldlist = { 'limits'        'real'     []                       0;
              ...
              'voffsets'      'real'     []                       [] ;};
[g topoargs] = finputcheck(options, fieldlist, 'timtopoemd', 'ignore');
...
if isnumeric(chan_locs) && chan_locs == 0,
 ...
 elseif length(g.limits) == 4
    ...
    fprintf('timtopoemd(): limits format not correct. See >> help timtopoemd.\n');
    return
  end;
  ...
  if xmax<=xmin,
      fprintf('timtopoemd() - in limits, maxms must be > minms.\n')
      return
  end
  ...
  if ymax<=ymin,
      fprintf('timtopoemd() - in limits, maxval must be > minmval.\n')
      return
  end
...
if plottimes_set == 1
  ...
  if ntopos > MAX_TOPOS
    fprintf('timtopoemd(): too many plottimes - only first %d will be shown!\n',MAX_TOPOS);
  ...
  if max(g.plottimes) > xmax | min(g.plottimes)< xmin
    fprintf(...
'timtopoemd(): at least one plottimes value outside of epoch latency range - cannot plot.\n');
    return
  end
...
for t=1:ntopos
  ...
  topoplotemd( data(:,plotframes(t)),chan_locs, topoargs{:});
...
% clicking on ERM pop_up topoplot
% -------------------------------
disp('Click on ERM waveform to show scalp map at specific latency');
...
cb_code = [ 'tmppos = get(gca, ''currentpoint'');' ...
            ...
            'topoplot(dattmp.erm(:,latpoint), dattmp.chanlocs, dattmp.options{:});' ...
