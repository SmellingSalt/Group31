% plottopoemd() - plot a specific mode of concatenated multichannel data epochs in a topographic
% or
...
% Usage:
%    >> plottopoemd(data, 'key1', 'val1', 'key2', 'val2')
% Or
%    >> plottopoemd(data,'chan_locs',frames,limits,title,channels,...
%                      axsize,colors,ydir,vert) % old function call
% Inputs:
%   data       = data consisting of a mode of consecutive epochs of (chans,frames)
%                or (chans,frames,n)
%
% Optional inputs:
...
%                data.
%
%  This code is written by Scott Makeig and Arnaud Delorme, SCCN/INC/UCSD, La Jolla, 3-2-98 and modified by
%  Karema Al-Subari (Karema.Al-Subari@ur.de) and Saad Al-Baddai (Saad.Al-Baddai@ur.de).
%
% See also: plotdata(), topoplotemd()
...
function plottopoemd(data, varargin);
...
%
  ...
  if g.frames <=0,
    ...
    datasets=1;
  elseif g.frames==1,
    fprintf('plottopoemd: cannot plot less than 2 frames per trace.\n');
    return
    datasets=1;
  else
  ...
  if max(g.chans) > chans
    fprintf('plottopoemd(): max channel index > %d channels in data.\n',...
                       chans);
    return
  end
  if min(g.chans) < 1
    fprintf('plottopoemd(): min channel index (%g) < 1.\n',...
                       min(g.chans));
    return
  end;
  if length(g.chans)>MAXPLOTDATACHANS,
    fprintf('plottopoemd(): not set up to plot more than %d traces.\n',...
                       MAXPLOTDATACHANS);
    return
  end;

  if datasets>MAXPLOTDATAEPOCHS 
      fprintf('plottopoemd: not set up to plot more than %d epochs.\n',...
                       MAXPLOTDATAEPOCHS);
    return
  end;
  if datasets<1
      fprintf('plottopoemd: cannot plot less than 1 epoch!\n');
      return
  end;
...
%             if chid <3,
%                 fprintf('plottopoemd(): cannot open file %s.\n',g.chans);
%                 return
%             else
%                 fprintf('plottopoemd(): opened file %s.\n',g.chans);
%             end;
% 
%             %%%%%%%
%             % fid=fopen('fgetl.m');
...
%     
    ...
    if g.limits==0,      % == 0 or [0 0 0 0]
        ...
        if length(g.limits)~=4,
            error('plottopoemd: limits should be 0 or an array [xmin xmax ymin ymax].\n');
        end;
        xmin = g.limits(1);
        xmax = g.limits(2);
        ymin = g.limits(3);
    ...
    if xmax<=xmin,
        fprintf('plottopoemd() - xmax must be > xmin.\n')
        return
    end
    ...
    if ymax<=ymin,
        fprintf('plottopoemd() - ymax must be > ymin.\n')
        return
    end
    ...
    figure(curfig); h=gca;title([g.title '  ERM'],'FontSize',TITLEFONTSIZE); % title plot 
    hold on
    ...
    if plotgrid
        ...
        wd = g.geom(2);
        if chans > ht*wd
            fprintf('plottopoemd(): (%d) channels to be plotted > grid size [%d %d]\n',...
                    chans,ht,wd);
            return
        end
        xvals = 0; yvals = 0;
        ...
        if length(g.chans) > length(g.chanlocs),
            error('plottopoemd(): data channels must be <= ''chanlocs'' channels')
        end
