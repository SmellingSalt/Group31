%  ermimageemd() - Plot a colored image of a collection of single-trial data epochs of a specific mode, optionally 
...
%              or phase within a specified latency window. Optionally plot the ERM mean 
...
% Usage:
%            >> figure;  ermimageemd(data,[],times); % image trials in input order
%
%            >> figure; [outdata,outvar,outtrials,limits,axhndls, ...
%                        erm,amps,cohers,cohsig,ampsig,outamps,...
%                        phsangls,phsamp,sortidx,ermsig] ...
%                            =  ermimageemd(data,sortvar,times,'title',avewidth,decimate,...
%                                  'key', 'val', ...); % use options
% Required input:
%   data     = [vector or matrix] Single mode of a single-channel input data to image.
...
%               smoothing}
%   decimate = Factor to decimate|intermolate ntrials by (may be non-integer)
...
%   'timewarp' = {[events], [warpms], {colors}} Time warp ERM, amplitude and phase
...
%   'noshow' = ['on'|'off'] Do not plot  ermimageemd, simply return outputs {default: 'off'}
%
% Sort data epochs:
...
%                  to end_ms by 10 ms.
%  'showwin'     = ['on'|'off'] Show sorting window behind ERM trace. {default: 'off'}
%
...
%   'limits'         = [lotime hitime minerm maxerm lodB hidB locoher hicoher basedB]
...
%                      ERMimage and ERMs (not smoothing).  Cannot be used
...
%
% Add epoch-mean ERM to plot:
%   'erm'      = ['on'|'off'|1|2|3|4] Plot ERM time average of the trials below the 
%                image.  If 'on' or 1, a single ERM (the mean of all trials) is shown.  If 2,
%                two ERMs (super and sub median trials) are shown.  If 3, the trials are split into 
%                tertiles and their three ERMs are shown.  If 4, the trials are split into quartiles
%                and four ERMs are shown. Note, if you want negative voltage plotted up, change YDIR
%                to -1 in icadefs.m.  If 'ermalpha' option is used, any values of 'erm' greater than
%                1 will be reset to 1. {default no ERM plotted}
%   'ermalpha' = [alpha] Visualizes two-sided significance threshold (i.e., a two-tailed test) for the 
%                null hypothesis of a zero mean, symmetric distribution (range: [.001 0.1]). Thresholds 
%                are determined via a permutation test. Requires 'erm' to be a value other than 'off'.
%                If 'erm' is set  to a value greater than 1, it is reset to 1 to increase plot readability.
%                {default: no alpha significance thresholds plotted}
%   'ermstd'   = ['on'|'off'] Plot ERM +/- stdev. Requires 'erm' {default: no std. dev. plotted}
%   'erm_grid' = If 'erm_grid' is added as an option voltage axis dashed grid lines will be
%                 added to the ERM plot to facilitate judging ERM amplitude
%   'rmerm'    = ['on'|'off'] Subtract the average ERM from each trial before processing {default: no}
%
% Add time/frequency information:
%   'coher'  = [freq] Plot ERM average plus mean amplitude & coherence at freq (Hz)
...
%
% Add plot features:
%   'cbar'           = ['on'|'off'] Plot color bar to right of ERM-image {default no}
...
% 'yermlabel'         = ['string'] ERM ordinate axis label (default is ERM). Print uV with '\muV'
...
%                        vertical components in the ERMimage. See 'avewidth' argument
%                        description for more information. {default: boxcar}
% 'img_trialax_label' = ['string'] The label of the axis corresponding to trials in the ERMimage 
...
%                        for reaction time in msec) will appear on the trial axis of the  ermimageemd. Tick mark
...
%                        filtered. Not necessary if data have already been baselined and  ermimageemd 
...
%    axhndls  = vector of 1-7 plot axes handles (img,cbar,erm,amp,coh,topo,spec)
%        erm  = plotted ERM average
...
%     ermsig  = trial average significance levels [2,frames]
%
% Example:  >> figure; 
%               ermimageemd(data,RTs,[-400 256 256],'Test',1,1,...
%                            'erm','cbar','vert',-350);
%    Plots an ERM-image of 1-s data epochs sampled at 256 Hz, sorted by RTs, with 
%    title ('Test'), and sorted epochs not smoothed or decimated (1,1). Overmlots 
%    the (unsmoothed) RT latencies on the colored ERM-image. Also plots the 
%    epoch-mean (ERM), a color bar, and a dashed vertical line at -350 ms.
%
%  This code is written by Scott Makeig, Tzyy-Ping Jung & Arnaud Delorme,
%   CNL/Salk Institute, La Jolla, 3-2-1998 - and modified by
...
%% LOG COMMENTS %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [data,outsort,outtrials,limits,axhndls,erm,amps,cohers,cohsig,ampsig,allamps,phaseangles,phsamp,sortidx,ermsig] = ermimageemd(data,sortvar,times,titl,avewidth,decfactor,arg1,arg2,arg3,arg4,arg5,arg6,arg7,arg8,arg9,arg10,arg11,arg12,arg13,arg14,arg15,arg16,arg17,arg18,arg19,arg20,arg21,arg22,arg23,arg24,arg25,arg26,arg27,arg28,arg29,arg30,arg31,arg32,arg33,arg34,arg35,arg36,arg37,arg38,arg39,arg40,arg41,arg42,arg43,arg44,arg45,arg46,arg47,arg48,arg49,arg50,arg51,arg52,arg53,arg54,arg55,arg56,arg57,arg58,arg59,arg60,arg61,arg62,arg63,arg64,arg65,arg66)
...
warning off;
erm = []; amps = []; cohers = []; cohsig = []; ampsig = [];
allamps = []; phaseangles = []; phsamp = []; sortidx = [];
auxvar = []; ermsig = []; winloc = [];winlocs = [];
timeStretchColors = {};
...
YEXPAND = 1.3;      % expansion factor for y-axis about erm, amp data limits
...
MIN_ERMALPHA = 0.001; % significance bounds for ERM
MAX_ERMALPHA = 0.1;
...
erm_grid = NO; % add y-tick grids to ERM plot -DG
cbar_title = []; % title to add above ERMimage color bar (e.g., '\muV') -DG
img_ylab = 'Trials'; % make the ERMimage y-axis in units of the sorting variable -DG
img_ytick_lab = []; % the values at which tick marks will appear on the trial axis of the ERMimage (the y-axis by default).
...
erm_vltg_ticks=[]; %if non-empty, these are the voltage axis ticks for the ERM plot
...
Ermflag   = NO;     % don't show erm average by default
Ermstdflag= NO;
Ermalphaflag= NO;
Alignflag = NO;     % don't align data to sortvar by default
Colorbar  = NO;     % if YES, plot a colorbar to right of erm image
...
timeStretchRef = [];
timeStretchMarks = [];
tsurdata = []; % time-stretched data before smoothing (for timestretched-erm computation)
...
if nargin < 3 | isempty(times)
    if size(data,1)==1 || size(data,2)==1
        fprintf(' ermimageemd(): either input a times vector or make data size = (frames,trials).\n')
        return
    end
    times = 1:size(data,1);
    NoTimesPassed= 1;
...
if frames*ntrials ~= framestot
    help  ermimageemd
    fprintf(...
        '\n ermimageemd(); length of sortvar doesn''t divide number of data elements??\n')
    return
end
...
if length(times) ~= frames
    fprintf(...
        ' ermimageemd(): length(data)(%d) ~= length(sortvar)(%d) * length(times)(%d).\n\n',...
    ...
    while a < nargin % for each remaining Arg
        ...
        if Caxflag == YES
            if size(Arg,1) ~= 1 || size(Arg,2) > 2
                help  ermimageemd
                fprintf('\n ermimageemd(): caxis arg must be a scalar or (1,2) vector.\n');
                return
            end
            if size(Arg,2) == 2
                Caxis = Arg;
        ...
        elseif timestretchflag == YES % Added -JH
            ...
            if ~isempty(smr)
                 fprintf(' ermimageemd(): Timewarp event latencies not in ascending order in trial %d.\n',smr)
                 return
            end
        ...
        elseif Coherflag == YES
            if length(Arg) > 3 || length(Arg) < 1
                help  ermimageemd
                fprintf('\n ermimageemd(): coher arg must be size <= 3.\n');
                return
            end
            coherfreq = Arg(1);
            if size(Arg,2) == 1
            ...
            if size(Arg,2) == 3
                ...
                if alpha < 0 || alpha > 0.1
                    fprintf(' ermimageemd(): alpha value %g out of bounds.\n',alpha);
                    return
                end
            end
            Coherflag = NO;
            Ermflag = YES;  % plot amp, coher below erm time series
        elseif Topoflag == YES;
            if length(Arg) < 2
                help  ermimageemd
                fprintf('\n ermimageemd(): topo arg must be a list of length 2 or 3.\n');
                return
            end
            topomap = Arg{1};
            eloc_file = Arg{2};
        ...
        elseif Specflag == YES;
            if length(Arg) ~= 2
                help  ermimageemd
                fprintf('\n ermimageemd(): spec arg must be a list of length 2.\n');
                return
            end
            lospecHz = Arg(1);
            hispecHz = Arg(2);
        ...
        elseif Limitflag == YES
            %  [lotime hitime loerm hierm loamp hiamp locoher hicoher]
            if size(Arg,1) ~= 1 || size(Arg,2) < 2 || size(Arg,2) > 9
                    help  ermimageemd
                fprintf('\n ermimageemd(): limits arg must be a vector sized (1,2<->9).\n');
                return
            end
            if  ~isnan(Arg(1)) & (Arg(2) <= Arg(1))
                help  ermimageemd
                fprintf('\n ermimageemd(): time limits out of order or out of range.\n');
                return
            end
            if Arg(1) < min(times)
                Arg(1) = min(times); 
            ...
            if ~isnan(maxerm) & maxerm <= minerm
                help  ermimageemd
                fprintf('\n ermimageemd(): erm limits args out of order.\n');
                return
            end
            if length(Arg)> 4
                minamp = Arg(5);
            ...
            if maxamp <= minamp
                help  ermimageemd
                fprintf('\n ermimageemd(): amp limits args out of order.\n');
                return
            end
            if length(Arg)> 6
                mincoh = Arg(7);
            ...
            if maxcoh <= mincoh
                help  ermimageemd
                fprintf('\n ermimageemd(): coh limits args out of order.\n');
                return
            end
            if length(Arg)>8
        ...
        elseif Auxvarflag == YES;
            ...
            elseif isa(Arg,'cell')==YES
                fprintf(' ermimageemd(): auxvars argument must be a matrix or length-2 cell array.\n');
                return
            else
        ...
        elseif Horzflag == YES
            horzepochs = Arg;
            Horzflag = NO;
        elseif yermlabelflag == YES
            yermlabel = Arg;
            yermlabelflag = NO;
        elseif Signifflag == YES
            signifs = Arg; % [low_amp hi_amp coher]
            if length(signifs) ~= 3
                fprintf('\n ermimageemd(): signif arg [%g] must have 3 values\n',Arg);
                return
            end
            Signifflag = NO;
        elseif Allcohersflag == YES
            ...
            if size(data2) ~= size(data)
                fprintf(' ermimageemd(): allcohers data matrix must be the same size as data.\n');
                return
            end
            Allcohersflag = NO;
        elseif Phaseflag == YES
            n = length(Arg);
            if n > 5
                error(' ermimageemd(): Too many arguments for keyword ''phasesort''');
            end
            phargs = Arg;

            if phargs(3) < 0
                error(' ermimageemd(): Invalid negative frequency argument for keyword ''phasesort''');
            end
            if n>=4
                if phargs(4) < 0
                    error(' ermimageemd(): Invalid negative argument for keyword ''phasesort''');
                end
            end
            if min(phargs(1)) < times(1) | max(phargs(1)) > times(end)
                error(' ermimageemd(): time for phase sorting filter out of bound.');
            end
            if phargs(2) >= 100 | phargs(2) < -100
                error('%-argument for keyword ''phasesort'' must be (-100;100)');
            end
            if length(phargs) >= 4 & phargs(3) > phargs(4)
                error(' ermimageemd(): Phase sorting frequency range must be increasing.');
            end
            if length(phargs) == 5
                topphase = phargs(5);
            end
        ...
        elseif Sortwinflag == YES % 'ampsort' mean amplitude over a time window
            ...
            sortwinarg = Arg;
            if n > 2
                error(' ermimageemd(): Too many arguments for keyword ''sortwin''');
            end
            if min(sortwinarg(1)) < times(1) | max(sortwinarg(1)) > times(end)
                error(' ermimageemd(): start time for value sorting out of bounds.');
            end
            if n > 1
                if min(sortwinarg(2)) < times(1) | max(sortwinarg(2)) > times(end)
                    error(' ermimageemd(): end time for value sorting out of bounds.');
                end
            end
            if n > 1 & sortwinarg(1) > sortwinarg(2)
                error(' ermimageemd(): Value sorting time range must be increasing.');
            end
            Sortwinflag = NO;
        elseif Ampflag == YES % 'ampsort',[center_time,prcnt_reject,minfreq,maxfreq]
            n = length(Arg);
            if n > 4
                error(' ermimageemd(): Too many arguments for keyword ''ampsort''');
            end
            ampargs = Arg;

            % if ampargs(3) < 0
            %    error(' ermimageemd(): Invalid negative argument for keyword ''ampsort''');
            % end
            if n>=4
                if ampargs(4) < 0
                    error(' ermimageemd(): Invalid negative argument for keyword ''ampsort''');
            ...
            if ~isinf(ampargs(1))
                if min(ampargs(1)) < times(1) | max(ampargs(1)) > times(end)
                    error(' ermimageemd(): time for amplitude sorting filter out of bounds.');
                end
            end
            ...
            if length(ampargs) == 4 & abs(ampargs(3)) > abs(ampargs(4))
                error(' ermimageemd(): Amplitude sorting frequency range must be increasing.');
            end
            Ampflag = NO;
        ...
        elseif Valflag == YES % sort by potential value in a given window
            ...
            n = length(Arg);
            if n > 3
                error(' ermimageemd(): Too many arguments for keyword ''valsort''');
            end
            valargs = Arg;
            ...
            if n==3 & (~isnumeric(valargs(3)) | valargs(3)==0)
                error(' ermimageemd(): Value sorting direction must be +1 or -1.');
            end
            Valflag = NO;
        ...
        elseif Ermalphaflag == YES
            ermalpha = Arg(1);
            if ermalpha < MIN_ERMALPHA | ermalpha > MAX_ERMALPHA
                fprintf(' ermimageemd(): ermalpha value is out of bounds [%g, %g]\n',...
                    MIN_ERMALPHA,MAX_ERMALPHA);
                return
            end
            Ermalphaflag = NO;
        % -----------------------------------------------------------------------
       ...
       elseif strcmpi(Arg,'avg_type')
        ...
        elseif strcmpi(Arg,'erm')
            Ermflag = YES;
            erm_ptiles=1;
            if a < nargin, 
                Arg = eval(['arg' int2str(a+1-6)]); 
                if strcmpi(Arg, 'on'),     Ermflag = YES; erm_ptiles=1; a = a+1;
                elseif strcmpi(Arg, 'off') Ermflag = NO;  a = a+1;
                elseif strcmpi(Arg,'1') | (Arg==1) Ermlag = YES; erm_ptiles=1; a=a+1;
                elseif strcmpi(Arg,'2') | (Arg==2) Ermlag = YES; erm_ptiles=2; a=a+1;
                elseif strcmpi(Arg,'3') | (Arg==3) Ermlag = YES; erm_ptiles=3; a=a+1;
                elseif strcmpi(Arg,'4') | (Arg==4) Ermlag = YES; erm_ptiles=4; a=a+1;
                end;
            end;            
        elseif strcmpi(Arg,'rmerm')
            Rmerm = 'yes';
            if a < nargin, 
                Arg = eval(['arg' int2str(a+1-6)]); 
                if strcmpi(Arg, 'on'),     Rmerm = 'yes'; a = a+1;
                elseif strcmpi(Arg, 'off') Rmerm = 'no';  a = a+1;
                end;
            end;            
        ...
        elseif strcmpi(Arg,'ermstd')
            Ermstdflag = YES;
            if a < nargin, 
                Arg = eval(['arg' int2str(a+1-6)]); 
                if strcmpi(Arg, 'on'),     Ermstdflag = YES; a = a+1;
                elseif strcmpi(Arg, 'off') Ermstdflag = NO;  a = a+1;
                end;
            end;            
        ...
        elseif strcmp(Arg,'cycles')
            Cycleflag = YES;
        elseif strcmpi(Arg,'yermlabel')
            yermlabelflag = YES;
        elseif strcmpi(Arg,'srate')
            Srateflag = YES;
        elseif strcmpi(Arg,'erm_grid')
            erm_grid = YES;
        ...
        elseif strcmpi(Arg,'erm_vltg_ticks')
            if a < nargin,
                a = a+1;
                erm_vltg_ticks=eval(['arg' int2str(a-6)]);
            else
                error('Argument ''erm_vltg_ticks'' needs to be followed by a vector.');
            end          
        ...
        elseif strcmp(Arg,'signif') || strcmp(Arg,'signifs') || strcmp(Arg,'sig') || strcmp(Arg,'sigs')
            ...
            help  ermimageemd
            if isstr(Arg)
                fprintf('\n ermimageemd(): unknown arg %s\n',Arg);
            else
                fprintf('\n ermimageemd(): unknown arg %d, size(%d,%d)\n',a,size(Arg,1),size(Arg,2));
            end
            return
        end
    end % Arg
...
if   Caxflag == YES ...
        ...
        |Limitflag == YES
    help  ermimageemd
    fprintf('\n ermimageemd(): missing option arg.\n')
    return
end
if (Allampsflag | exist('data2')) & ( any(isnan(coherfreq)) | ~Cohsigflag )
    fprintf('\n ermimageemd(): allamps and allcohers flags require coher freq, srate, and cohsig.\n');
    return
end
if Allampsflag & exist('data2')
    fprintf('\n ermimageemd(): cannot image both allamps and allcohers.\n');
    return
end
if ~exist('srate') | srate <= 0
    fprintf('\n ermimageemd(): Data srate must be specified and > 0.\n');
    return
end
if ~isempty(auxvar)
    % whos auxvar
    if size(auxvar,1) ~= ntrials & size(auxvar,2) ~= ntrials
        fprintf(' ermimageemd(): auxvar size should be (N,ntrials), e.g., (N,%d)\n',...
            ntrials);
        return
    end
    ...
    if exist('auxcolors')==YES % if specified
        if isa(auxcolors,'cell')==NO % if auxcolors is not a cell array
            fprintf(...
                ' ermimageemd(): auxcolors argument to auxvar flag must be a cell array.\n');
            return
        end
    end
elseif exist('timeStretchRef') & ~isempty(timeStretchRef)
    if ~isnan(aligntime)
        fprintf([' ermimageemd(): options "align" and ' ...
...
% No need to turn off ERM anymore; we now time-stretch potentials
% separately (see tsurdata below) so the (warped) ERM is actually meaningful

if exist('phargs')
    if phargs(3) > srate/2
        fprintf(...
            ' ermimageemd(): Phase-sorting frequency (%g Hz) must be less than Nyquist rate (%g Hz).',...
            phargs(3),srate/2);
    end

    if frames < cycles*srate/phargs(3)
        fprintf('\n ermimageemd(): phase-sorting freq. (%g) too low: epoch length < %d cycles.\n',...
            phargs(3),cycles);
        return
    end
    ...
    if length(phargs)==5 & (phargs(5)>180 | phargs(5) < -180)
        fprintf('\n ermimageemd(): coher topphase (%g) out of range.\n',topphase);
        return
    end
end
if exist('ampargs')
    if abs(ampargs(3)) > srate/2
        fprintf(...
            ' ermimageemd(): amplitude-sorting frequency (%g Hz) must be less than Nyquist rate (%g Hz).',...
            abs(ampargs(3)),srate/2);
    end

    if frames < cycles*srate/abs(ampargs(3))
        fprintf('\n ermimageemd(): amplitude-sorting freq. (%g) too low: epoch length < %d cycles.\n',...
...
if ~any(isnan(coherfreq))
    if coherfreq(1) <= 0 | srate <= 0
        fprintf('\n ermimageemd(): coher frequency (%g) out of range.\n',coherfreq(1));
        return
    end
    ...
    if frames < cycles*srate/coherfreq(1)
        fprintf('\n ermimageemd(): coher freq. (%g) too low:  epoch length < %d cycles.\n',...
            coherfreq(1),cycles);
        return
    end
end
...
if ~isstr(aligntime) & ~isnan(aligntime)
    if ~isinf(aligntime) ...
        ...
        help  ermimageemd
        fprintf('\n ermimageemd(): requested align time outside of time limits.\n');
        return
    end
end
%
...
if strcmpi(mvavg_type,'Gaussian'),
    ...
    elseif avewidth < 1,
        help  ermimageemd
        fprintf('\n ermimageemd(): Variable avewidth cannot be < 1.\n')
        fprintf('\n ermimageemd(): avewidth needs to be a positive integer.\n')
        return
    end
...
if ~isempty(flt)
    ...
    if isempty(baseline)
        fprintf('Note, you might want to re-baseline the data using the ermiamge ''baseline'' option.\n\n');
    end
end
...
switch lower(renorm)
    case 'yes',
        disp(' ermimageemd warning: *** sorting variable renormalized ***');
    ...
    otherwise,
        if ~isempty(renorm)
            ...
            if length(locx) ~= 1, error(' ermimageemd: unrecognized renormalizing formula'); end;
...
if exist('phargs') == 1 % if phase-sort the data trials
    ...
    % $$$       fprintf(...
    % $$$           '    Filter length is %d; Phase has been linearly intermolated to latency at %1.0f ms.\n', ...
...
elseif exist('ampargs') == 1 % if amplitude-sort
    ...
    if length(tmprange) ~=  winlen+1 % ?????????
        ...
        fprintf(...
            '    Wavelet length is %d; Phase has been linearly intermolated to latency et %1.0f ms.\n', ...
...
if strcmpi(noshow, 'no')
    ...
    elseif Ermflag == YES   % elseif if plot only one time axes
        image_loy = 1*PLOT_HEIGHT;
    else                    % else plot erm-image only
...
if ~Allampsflag & ~exist('data2') %%%%%%%% Plot ERM image %%%%%%%%%%
...
elseif Allampsflag %%%%%%%%%%%%%%%% Plot allamps instead of data %%%%%%%%%%%%%%
    ...
    if ~isempty(Caxis)
        ...
        % Changed -JH
        % 0.8 is Scott's suggestion to make the erm image show small
        % variations better
...
elseif exist('data2') %%%%%% Plot allcohers instead of data %%%%%%%%%%%%%%%%%%%
    ...
    if ~exist('allcohers')
        fprintf(' ermimageemd(): allcohers not returned....\n')
        return
    end
...
%Change limits on ERMimage y-axis if requested
if ~isempty(sortvar_limits)
    if exist('phargs','var'),
        fprintf('********* Warning *********\n');
...
if ~isempty(img_ylab) && ~strcmpi(img_ylab,'Trials')
    %make ERMimage y-tick labels in units of sorting variable
    if isempty(sortvar_limits),
        mn=min(outsort);
        mx=max(outsort);
    else
...
elseif ~isempty(img_ylab), %make ERMimage y-tick labels in units of Trials
    if isempty(img_ytick_lab)
        v=axis; %note: sorting variable limits have already been used to determine range of ERMimage y-axis
        mn=v(3);
        mx=v(4);
        ord=orderofmag(mx-mn);
        rng_rnd=round([mn mx]/ord)*ord;
...
if ~isempty(verttimes)
    ...
    if size(verttimes,1) ~= 1 & size(verttimes,1) ~= ntrials
        fprintf('\n ermimageemd(): vert arg matrix must have 1 or %d rows\n',ntrials);
        return
    end;
    if strcmpi(noshow, 'no')
...
if ~isempty(horzepochs)
    if size(horzepochs,1) > 1 & size(horzepochs,1) > 1
        fprintf('\n ermimageemd(): horz arg must be a vector\n');
        return
    end;
    if strcmpi(noshow, 'no')
        if ~isempty(img_ylab) && ~strcmpi(img_ylab,'Trials'),
            ...
            for he = horzepochs % for each horizontal line             
                ...
                if (he>mn) && (he<mx)
                    ...
                    if TIMEX          % overmlot he_ep on image
...
if strcmpi(noshow, 'no')
    ...
    if Noshow == YES
        fprintf('Not overmlotting sorted sortvar on data.\n');

    elseif isnan(aligntime) % plot sortvar on un-aligned data

        if Nosort == NO;
            fprintf('Overmlotting sorted sortvar on data.\n');
        end
        hold on;
        if TIMEX      % overmlot sortvar
        ...
        hold on;
        if TIMEX      % overmlot re-aligned 0 time on image
            figure(curfig);plot([aligntime aligntime],[min(outtrials) max(outtrials)],...
        ...
        fprintf('Overmlotting realigned times-zero on data.\n');
        hold on;

        if TIMEX      % overmlot realigned sortvar on image
...
if strcmpi(noshow, 'no')
    if ~isempty(auxvar)
        fprintf('Overmlotting auxvar(s) on data.\n');
        hold on;
        ...
        for c=1:size(auxvar,1)
            ...
            if ~isempty(auxcolor)
              if isnan(aligntime) % plot auxvar on un-aligned data
                  ...
                  if TIMEX      % overmlot realigned 0-time on image
...
if strcmpi(noshow, 'no')
    if Colorbar == YES
        ...
        % drawnow
        axes(ax1); % reset current axes to the  ermimageemd
    end
end;
...
if Ermflag == YES
    if exist('ermalpha')
        if erm_ptiles>1,
           fprintf(['\nOnly plotting one ERM (i.e., not plotting ERMs from %d percentile splits) ' ...
               'because ''ermalpha'' option was chosen.  You can''t plot both.\n\n'],erm_ptiles)
        end
        [erm ermsig] = nan_mean(fastif(length(tsurdata) > 0, tsurdata',urdata'), ...
            ermalpha);
        fprintf('   Mean ERM (p<%g) significance threshold: +/-%g\n', ...
            ermalpha,mean(ermsig));
    else
        %potentially make ERMs of 50%, 33%, or 25% split of trials 
        n_trials=size(urdata,2);
        trials_step=round(n_trials/erm_ptiles);
        erm=zeros(erm_ptiles,size(urdata,1));
        for ploop=1:erm_ptiles,
            ...
            if length(tsurdata) > 0
                erm(ploop,:) = nan_mean(tsurdata(:,ptile_trials)');
            else
                erm(ploop,:) = nan_mean(urdata(:,ptile_trials)');
            end;
        end
       % else
            %orig line
            %[erm] = nan_mean(fastif(length(tsurdata) > 0, tsurdata', urdata'));
        %end
    end % compute average ERM, ignoring nan's
end;

if Ermflag == YES & strcmpi(noshow, 'no')
    axes(ax1); % reset current axes to the  ermimageemd
    ...
    %
    %%%%%% Plot ERM time series below image %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %
    if isnan(maxerm)
        fac = 10;
        maxerm = 0;
        while maxerm == 0
            maxerm = round(fac*YEXPAND*max(erm))/fac; % minimal decimal places
            fac = 10*fac;
        end
        if Ermstdflag == YES
            fac = fac/10;
            maxerm = max(maxerm, round(fac*YEXPAND*max(erm+stdev))/fac);
        end;
        if ~isempty(ermsig)
            ermsig = [ermsig;-1*ermsig];
            maxerm = max(maxerm, round(fac*YEXPAND*max(ermsig))/fac);
        end
        maxerm=max(maxerm);
    end
    if isnan(minerm)
        ...
        minerm = 0;
        while minerm == 0
            minerm = round(fac*YEXPAND*min(erm))/fac; % minimal decimal places
            fac = 10*fac;
        end
        if Ermstdflag == YES
            fac = fac/10;
            minerm = min(minerm, round(fac*YEXPAND*min(erm-stdev))/fac);
        end;
        if ~isempty(ermsig)
            minerm = min(minerm, round(fac*YEXPAND*min(ermsig))/fac);
        end
        minerm=min(minerm);
    end
    limit = [timelimits(1:2) minerm maxerm];
    ...
    fprintf('Plotting the ERM trace below the ERM image\n');
    if Ermstdflag == YES
        if Showwin
            tmph = plot1trace(ax2,times,erm,limit,[],stdev,times(winlocs),erm_grid,erm_vltg_ticks); % plot ERM +/-stdev
        else
            tmph = plot1trace(ax2,times,erm,limit, [], stdev,[],erm_grid,erm_vltg_ticks); % plot ERM +/-stdev
        end
    elseif ~isempty('ermsig')
        if Showwin
            tmph = plot1trace(ax2,times,erm,limit,ermsig,[],times(winlocs),erm_grid,erm_vltg_ticks); % plot ERM and 0+/-alpha threshold
        else
            tmph = plot1trace(ax2,times,erm,limit,ermsig,[],[],erm_grid,erm_vltg_ticks); % plot ERM and 0+/-alpha threshold
        end
    else % plot ERM alone - no significance or std dev plotted
        if Showwin
            tmph = plot1trace(ax2,times,erm,limit,[],[],times(winlocs),erm_grid,erm_vltg_ticks); % plot ERM alone
        else
            tmph = plot1trace(ax2,times,erm,limit,[],[],[],erm_grid,erm_vltg_ticks); % plot ERM alone
        end
    end;
    ...
    set(ax2,'Xtick',xtick);        % use same Xticks as  ermimageemd above
    if ~isnan(coherfreq)
        set(ax2,'Xticklabel',[]);    % remove tick labels from ERM x-axis
    else % bottom axis
        set(ax2,'Xticklabel',xticklabel); % add ticklabels to ERM x-axis
    end
    ...
    if ~isempty(verttimes)
        ...
        for vt = vts
            if isnan(aligntime)
                ...
                if TIMEX      % overmlot realigned vt on ERM
    ...
    t=text(ytextoffset,ynum,yermlabel,'Rotation',90);
...
if ~isnan(coherfreq)
    ...
    if strcmpi(noshow, 'no')
        axis('off') % rm ERM axes axis and labels
        ...
        if ~exist('ynumoffset')
            limit = [timelimits(1:2) -max(abs([minampERM maxampERM])) max(abs([minampERM maxampERM]))];
            limit = double(limit);
            ydelta = double(1/10*(limit(2)-limit(1)));
            ytextoffset = double(limit(1)-1.1*ydelta);
            ynumoffset  = double(limit(1)-0.3*ydelta); % double for Matlab 7
        ...
        t=text(ynumoffset,maxampERM,num2str(maxampERM,3));
        set(t,'HorizontalAlignment','right','FontSize',TICKFONT);

        t=text(ynumoffset,minampERM, num2str(minampERM,3));
        set(t,'HorizontalAlignment','right','FontSize',TICKFONT);    
        ...
        fprintf('Plotting the ERSP amplitude trace below the ERM\n');
        ...
        if ~isempty(verttimes)
            ...
            for vt = vts
                if isnan(aligntime)
                    ...
                    if TIMEX      % overmlot realigned vt on amp
        ...
        if ~isempty(verttimes)
          ...
          for vt = vts
              if isnan(aligntime)
               ...
               if TIMEX      % overmlot realigned vt on coher
                figure(curfig);plot(repmat(median(aligntime+vt-outsort),1,2),...
                                                   [mincoh,maxcoh],DOTSTYLE,'LineWidth',VERTWIDTH);
               else
...
%
limits = [min(times) max(times) minerm maxerm minamp maxamp mincoh maxcoh];
...
%% %%%%%%%%%%%%%%%%% function plot1trace() %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
function [plot_handle] = plot1trace(ax,times,trace,axlimits,signif,stdev,winlocs,erm_grid,erm_vltg_ticks)
%function [plot_handle] = plot1trace(ax,times,trace,axlimits,signif,stdev,winlocs,erm_grid,erm_vltg_ticks)
...
if nargin<9,
    erm_vltg_ticks=[]; %if erm_vltg_ticks is not empty, those will be the ticks used on the y-axis (voltage axis for ERMs)
end
...
for traceloop=1:size(trace,1),
    [plot_handle(traceloop)] = plot(times,trace(traceloop,:),'LineWidth',ERMDATAWIDTH, ...
...
if ~isempty(axlimits) && sum(isnan(axlimits))==0
    ...
    l1=line([axlimits(1:2)],[0 0],    'Color','k',...
        'linewidth',ERMZEROWIDTH); % y=zero-line
    timebar=0; 
    l2=line([1 1]*timebar,axlimits(3:4)*1.1,'Color','k',...
        'linewidth',ERMZEROWIDTH); % x=zero-line

    %y-ticks
    if isempty(erm_vltg_ticks),
        ...
        if (sign(shrunk_ylimits(2))*sign(shrunk_ylimits(1)))==1, %y shrunk_ylimits don't include 0
            erm_yticks=shrunk_ylimits(1):ystep:shrunk_ylimits(2);
        else %y limits include 0
            % erm_yticks=0:ystep:shrunk_ylimits(2); %ensures 0 is a tick point
            % erm_yticks=[erm_yticks [0:-ystep:shrunk_ylimits(1)]];
            erm_yticks=0:ystep:axlimits(4); %ensures 0 is a tick point
            erm_yticks=[erm_yticks [0:-ystep:axlimits(3)]];
        end
        if erm_grid,
            set(gca,'ytick',unique(erm_yticks),'ygrid','on');
        else
            set(gca,'ytick',unique(erm_yticks));
        end
    else    
        set(gca,'ytick',erm_vltg_ticks); 
    end
end

%make ERM traces on top
...
%% %%%%%%%%%%%%%%%%% function phasedet() %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% phasedet() - function used in  ermimageemd.m
...
if nargin>1
    ...
    fprintf('Performing a permuration test using %d permutations to determine ERM significance thresholds... ',NPERM);
    permerms = zeros(NPERM,inframes);
    for n=1:NPERM
        signs = sign(randn(1,intrials)'-0.5);
        permerms(n,:) = sum(repmat(signs,1,inframes).*in)./nonnans;
        if ~rem(n,50)
            fprintf('%d ',n);
        end
    end
    fprintf('\n');
    permerms = sort(abs(permerms));
    alpha_bnd = floor(2*alpha*NPERM); % two-sided probability threshold
    outalpha = permerms(end-alpha_bnd,:);
end
out(nononnans) = NaN;
%
%% %%%%%%%%%%%%%%%%%%%%% function nan_std() %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%               ERMimage (this will just be 1:n_trials if there's no
%               smoothing)
%
% Output:
...
% Note: targ and vals should be in the same units (e.g., milliseconds)
    ...
    abv=find(vals>=targ);
    if isempty(abv),
        %point lies outside of vals range, can't intermolate
        y_pt=[];
        return
    end
    abv=abv(1);
